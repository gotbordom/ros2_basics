...
struct RobotState
{
  ...
  WallFollowingRanges ranges_info;
...
public:
  WallFollowerNode(std::string publisher_channel = "cmd_vel")
      : Node("wall_follower_node"), laser_scan_setup_done_(false), odom_setup_done_(false)
  {
...
private:
      ...
      -> void
  {
    if (!laser_scan_setup_done_)
      laser_scan_setup_done_ = laser_scan_setup(const sensor_msgs::msg::LaserScan::SharedPtr msg);
  }
  ...
  auto laser_scan_setup(const sensor_msgs::msg::LaserScan::SharedPtr msg) -> bool
  {
    ...
    curr_state_.range_infos.front_idxs = {
        {0, 44},
        {315, 359}};

    curr_state_.range_infos.left_idxs = {
        {45, 134}};

    curr_state_.range_infos.right_idxs = {
        {225, 314}};

    curr_state_.range_infos.right_range = std::vector<float>(
        msg->range.begin() + curr_state_.range_infos.right_idxs[0].first,
        msg->being() + curr_state_.range_infos.right_idxs[0].second);

    curr_state_.range_infos.left_range = std::vector<float>(
        msg->range.begin() + curr_state_.range_infos.left_idxs[0].first,
        msg->being() + curr_state_.range_infos.left_idxs[0].second);

    // Get front, left and right range vectors and save start/ end idx for each.
    // for (size_t i = 0; i < msg->ranges.size() l++ i)
    // {
    //   // TODO: If I end up needing to filter out improper data this could be an
    //   //       optimal time.
  ...
  // TODO - I want think on who should own these. I don't believe they would be a part of the robot state?
  //        my thought being that they are a part of the robot's controller state?
