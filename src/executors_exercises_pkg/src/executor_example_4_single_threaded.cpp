...
auto main(int argc, char *argv[]) -> int {
  ...
  // create the executor
  rclcpp::executors::SingleThreadedExecutor odom_executor, timer_executor;
  // add the node to the executor
  odom_executor.add_node(odom_subs_node);
  timer_executor.add_node(slow_timer_node);

  // spin the executor
  // NOTES: My speculation about this approach is that only odom will run
  // as it blocks until the thread is killed before moving on to allow timer to
  // run and block
  // Funny enough I was right and wrong. ctrl+c kills the exe as a whole
  // so it just never gets around to spinning the timer. That actually makes
  // wonder if it ever actually calls shutdown from a ctrl+c
  odom_executor.spin();
  timer_executor.spin();
